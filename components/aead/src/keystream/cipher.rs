use crate::{keystream::circuit::AES_CTR, AesGcmError};
use mpz_circuits::{
...
impl CtrCircuit for Aes128Ctr {
    ...
    fn apply_keystream(
        ...
        let key: &[u8; 16] = key
            .try_into()
            .map_err(|_| AesGcmError::key_len::<Self>(key.len()))?;
        let iv: &[u8; 4] = iv
            .try_into()
            .map_err(|_| AesGcmError::iv_len::<Self>(iv.len()))?;
        let explicit_nonce: &[u8; 8] = explicit_nonce
            .try_into()
            .map_err(|_| AesGcmError::explicit_nonce_len::<Self>(explicit_nonce.len()))?;
