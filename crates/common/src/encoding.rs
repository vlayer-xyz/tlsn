...
pub async fn receive(
...
) -> Result<impl EncodingProvider, String> {
    let Encodings { mut sent, mut recv } = ctx.io_mut().expect_next().await.unwrap();
    ...
    assert_eq!(sent.len(), sent_macs.len());
    assert_eq!(recv.len(), recv_macs.len());
