...
pub trait P128Pow5T16Constants: FromUniformBytes<64> + Ord {
    fn partial_rounds() -> usize {
        64
    }
    fn round_constants() -> Vec<[Self; 16]>;
    fn mds() -> Mds<Self, 16>;
    fn mds_inv() -> Mds<Self, 16>;
}

#[derive(Debug, Copy, Clone)]
pub struct P128Pow5T16<C> {
    _marker: PhantomData<C>,
}

impl<F: P128Pow5T16Constants> Spec<F, 16, 15> for P128Pow5T16<F> {
...
mod tests {
    ...
    use ::primitives::{permute, CachedSpec, ConstantLength, Hash, Spec};

    type F = halo2_proofs::halo2curves::bn256::Fr;

    /// The same Poseidon specification as poseidon::P128Pow5T16, but constructed
    /// such that its constants will be generated at runtime.
    #[derive(Debug, Copy, Clone)]
    pub struct P128Pow5T16Gen<F: PrimeField, const SECURE_MDS: usize>(PhantomData<F>);

    type P128Pow5T16Pasta = super::P128Pow5T16<F>;\

    impl<F: PrimeField, const SECURE_MDS: usize> P128Pow5T16Gen<F, SECURE_MDS> {
        pub fn new() -> Self {
            P128Pow5T16Gen(PhantomData::default())
        }
    }

    impl<F: FromUniformBytes<64> + Ord, const SECURE_MDS: usize> Spec<F, 16, 15>
        for P128Pow5T16Gen<F, SECURE_MDS>
    {
        fn full_rounds() -> usize {
            8
        }
    ...
    impl CachedSpec<F, 16, 15> for P128Pow5T16Gen<F,0> {
        fn cached_round_constants() -> &'static [[F; 16]] {
            &F::ROUND_CONSTANTS
        }
    ...
    fn verify_constants() {
        fn verify_constants_helper<F: FromUniformBytes<64> + Ord>(
            expected_round_constants: [[F; 16]; 64],
            expected_mds: [[F; 16]; 16],
            expected_mds_inv: [[F; 16]; 16],
        ) {
            let (round_constants, mds, mds_inv) = P128Pow5T16Gen::<F, 0>::constants();
    ...
    fn test_against_reference() {
        {
            ...
            permute::<F, P128Pow5T16Gen<F, 0>, 16, 15>(&mut input, &F::MDS, &F::ROUND_CONSTANTS);
        ...
        /*        {
                    ...
                    permute::<Fq, P128Pow5T16Gen<Fq, 0>, 3, 2>(&mut input, &fq::MDS, &fq::ROUND_CONSTANTS);
                    assert_eq!(input, expected_output);
                }
        */
    }
