...
<F: P128Pow5T3Constants> Spec<F, 3, 2> for P128Pow5T3<F> {
...
mod tests {
    ...
    impl CachedSpec<F, 3, 2> for P128Pow5T3Pasta {
    ...
    fn test_against_reference() {
        {
            ...
            permute::<F, P128Pow5T3Gen<F, 0>, 3, 2>(&mut input, &fp::MDS, &fp::ROUND_CONSTANTS);
        ...
        /*        {
                    ...
                    permute::<Fq, P128Pow5T2Gen<Fq, 0>, 3, 2>(&mut input, &fq::MDS, &fq::ROUND_CONSTANTS);
                    assert_eq!(input, expected_output);
                }
        */
    }
    ...
    fn permute_test_vectors() {
        {
            ...
            for tv in test_vectors::fp::permute() {
                let mut state = [
                    ...
                    Fp::from_repr(tv.initial_state[3]).unwrap(),
                    Fp::from_repr(tv.initial_state[4]).unwrap(),
                    Fp::from_repr(tv.initial_state[5]).unwrap(),
                    Fp::from_repr(tv.initial_state[6]).unwrap(),
                    Fp::from_repr(tv.initial_state[7]).unwrap(),
                    Fp::from_repr(tv.initial_state[8]).unwrap(),
                    Fp::from_repr(tv.initial_state[9]).unwrap(),
                    Fp::from_repr(tv.initial_state[10]).unwrap(),
                    Fp::from_repr(tv.initial_state[11]).unwrap(),
                    Fp::from_repr(tv.initial_state[12]).unwrap(),
                    Fp::from_repr(tv.initial_state[13]).unwrap(),
                    Fp::from_repr(tv.initial_state[14]).unwrap(),
                    Fp::from_repr(tv.initial_state[15]).unwrap(),
                    Fp::from_repr(tv.initial_state[16]).unwrap(),
                ];

                permute::<F, P128Pow5T3Pasta, 3, 2>(&mut state, &mds, &round_constants);
