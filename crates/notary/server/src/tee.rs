...
pub async fn quote() -> Quote {
    let tee_cache = Arc::new(Mutex::new(TeeState {
        measurement_ready: Mutex::new(false), // Wrap in a Mutex for concurrency safety
        cached_content: None,
        file_path: "output_file.txt".to_string(),
    }));

    cached_quote(tee_cache.clone()).await
}

// Example struct to store the cached state
struct TeeState {
    measurement_ready: Mutex<bool>, // Wrap the bool in a Mutex
    cached_content: Option<Quote>,  // Cached file content
    file_path: String,              // Path to the file
}

// Example Quote type

// Single function that handles caching and file serving
async fn cached_file_logic(state: Arc<Mutex<TeeState>>) -> Result<Quote, String> {
    let ready: bool;
    {
        let state_guard = state.lock().await;
        let measurement_ready_guard = state_guard.measurement_ready.lock().await;
        ready = *measurement_ready_guard;
    } // The lock on `measuremen

    if ready {
        // Re-lock `state` for immutable access to `cached_content`
        let state_guard = state.lock().await;
        if let Some(_content) = &state_guard.cached_content {
            return Ok(Quote::default()); // Return a default Quote or some
                                         // meaningful result
        }
    }

    // If the file is not ready, generate it and cache the result
    let content = generate_expensive_file().await;
    let mut state_guard = state.lock().await;
    match write_to_file(
        &state_guard.file_path,
        serde_json::to_string(&content).unwrap(),
    )
    .await
    {
        Ok(_) => {
            // Mark the file as ready and cache the content
            *state_guard.measurement_ready.lock().await = true; // Set measurement_ready to true
            state_guard.cached_content = Some(content);
            Ok(Quote::default())
        }
...
async fn generate_expensive_file() -> Quote {
...
async fn write_to_file(path: &str, content: String) -> Result<(), IoError> {
    fs::write(path, content)
}

async fn cached_quote(tee_cache: Arc<Mutex<TeeState>>) -> Quote {
    match cached_file_logic(tee_cache).await {
        Ok(quote) => quote,
        Err(e) => {
            println!("Error: {}", e);
            Quote::default()
