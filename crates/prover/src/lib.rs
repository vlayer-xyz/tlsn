...
fn build_mpc_tls(config: &ProverConfig, ctx: Context) -> (Arc<Mutex<Deap<Mpc, Zk>>>, MpcTlsLeader) {
    ...
    let mpc = Mpc::new(
        mpz_ot::cot::DerandCOTSender::new(rcot_send.next().unwrap()),
        rng.gen(),
        delta,
    );

    let zk = Zk::new(rcot_recv.next().unwrap());
    ...
    (
        ...
        MpcTlsLeader::new(
            ...
            vm,
            (
                rcot_send.next().unwrap(),
                rcot_send.next().unwrap(),
                rcot_send.next().unwrap(),
            ),
            rcot_recv.next().unwrap(),
        ),
    )
}
